...
class LLaVAMLLMBackbone(BaseMLLMBackbone):
    ...
    def preprocess_batch(
        ...
        # 3. 批量调用processor（HuggingFace processor支持批量输入）
        # 将图像转换为RGB格式
        rgb_images = [img.convert("RGB") for img in resized_images]

        # 批量处理：processor内部会自动处理padding
        inputs = self.processor(
            text=prompts,  # List[str]
            images=rgb_images,  # List[PIL.Image]
            return_tensors="pt",
            padding=True,  # 自动右padding到batch内最大长度
            padding_side="right"
        )
